...
namespace fea
{
    namespace util
    {
        void addBasicDataTypes(EntityFactory& factory)
        {
            ...
            {
                FEA_ASSERT(params.size() == 1, "Wrong amount of parameters! Expected 1, got " + std::to_string(params.size()) + "!");
                FEA_ASSERT(params[0][0] != '-', "Default value for type uint8 is not allowed to be negative! Given '" + params[0] + "'.");
            ...
            {
                FEA_ASSERT(params.size() == 1, "Wrong amount of parameters! Expected 1, got " + std::to_string(params.size()) + "!");
                FEA_ASSERT(params[0][0] != '-', "Default value for type uint16 is not allowed to be negative! Given '" + params[0] + "'.");
            ...
            {
                FEA_ASSERT(params.size() == 1, "Wrong amount of parameters! Expected 1, got " + std::to_string(params.size()) + "!");
                FEA_ASSERT(params[0][0] != '-', "Default value for type uint32 is not allowed to be negative! Given '" + params[0] + "'.");
            ...
            {
                FEA_ASSERT(params.size() == 1, "Wrong amount of parameters! Expected 1, got " + std::to_string(params.size()) + "!");
                FEA_ASSERT(params[0][0] != '-', "Default value for type uint64 is not allowed to be negative! Given '" + params[0] + "'.");
