...
int stbtt_InitFont(stbtt_fontinfo *info, const unsigned char *data2, int fontstart)
{
   ...
   cmap = (stbtt_uint32)stbtt__find_table(data, (stbtt_uint32)fontstart, "cmap");       // required
   ...
   for (i=0; i < numTables; ++i) {
      stbtt_uint32 encoding_record = cmap + 4 + 8 * (stbtt_uint32)i;
         ...
         case STBTT_PLATFORM_ID_MICROSOFT:
            switch (ttUSHORT(data+encoding_record+2)) {
               ...
               case STBTT_MS_EID_UNICODE_FULL:
                  // MS/Unicode
                  info->index_map = (stbtt_int32)(cmap + ttULONG(data+encoding_record+4));
                  break;
            }
            break;
      }
...
int stbtt_FindGlyphIndex(const stbtt_fontinfo *info, int unicode_codepoint)
{
   ...
   if (format == 0) { // apple byte encoding
      ...
      if ((stbtt_uint32) unicode_codepoint >= first && (stbtt_uint32) unicode_codepoint < first+count)
         return ttUSHORT(data + index_map + 10 + ((stbtt_uint32)unicode_codepoint - first)*2);
      return 0;
   } else if (format == 2) {
      ...
      while (low < high) {
         ...
         if ((stbtt_uint32) unicode_codepoint < start_char)
            ...
            if (format == 12)
               return (stbtt_int32)(start_glyph + (stbtt_uint32)unicode_codepoint-start_char);
            else // format == 13
               return (stbtt_int32)start_glyph;
         }
      }
      return 0; // not found
   }
...
static int stbtt__GetGlyfOffset(const stbtt_fontinfo *info, int glyph_index)
{
   ...
   if (info->indexToLocFormat == 0) {
      g1 = (stbtt_int32)((stbtt_uint32)info->glyf + ttUSHORT(info->data + info->loca + glyph_index * 2) * 2);
      g2 = (stbtt_int32)((stbtt_uint32)info->glyf + ttUSHORT(info->data + info->loca + glyph_index * 2 + 2) * 2);
   } else {
      g1 = (stbtt_int32)((stbtt_uint32)info->glyf + ttULONG (info->data + info->loca + glyph_index * 4));
      g2 = (stbtt_int32)((stbtt_uint32)info->glyf + ttULONG (info->data + info->loca + glyph_index * 4 + 4));
   }
...
int stbtt_GetGlyphShape(const stbtt_fontinfo *info, int glyph_index, stbtt_vertex **pvertices)
{
   ...
   *pvertices = NULL;
   ...
   if (numberOfContours > 0) {
      ...
      vertices = (stbtt_vertex *) STBTT_malloc((unsigned long)m * sizeof(vertices[0]), info->userdata);
      if (vertices == 0)
         return 0;
      ...
      while (more) {
         ...
         if (comp_num_verts > 0) {
            ...
            // Append vertices.
            tmp = (stbtt_vertex*)STBTT_malloc(((unsigned long)(num_vertices+comp_num_verts))*sizeof(stbtt_vertex), info->userdata);
            if (!tmp) {
               if (vertices) STBTT_free(vertices, info->userdata);
               if (comp_verts) STBTT_free(comp_verts, info->userdata);
               return 0;
            }
            if (num_vertices > 0) memcpy(tmp, vertices, (unsigned long)num_vertices*sizeof(stbtt_vertex));
            memcpy(tmp+num_vertices, comp_verts, (unsigned long)comp_num_verts*sizeof(stbtt_vertex));
            if (vertices) STBTT_free(vertices, info->userdata);
            vertices = tmp;
...
static void stbtt__rasterize_sorted_edges(stbtt__bitmap *result, stbtt__edge *e, int n, int vsubsample, int off_x, int off_y, void *userdata)
{
   ...
   if (result->w > 512)
      scanline = (unsigned char *) STBTT_malloc((size_t)result->w, userdata);
   else
   ...
   while (j < result->h) {
      ...
      STBTT_memcpy(result->pixels + j * result->stride, scanline, (size_t)result->w);
      ++j;
   }
...
static void stbtt__rasterize(stbtt__bitmap *result, stbtt__point *pts, int *wcount, int windings, float scale_x, float scale_y, float shift_x, float shift_y, int off_x, int off_y, int invert, void *userdata)
{
   ...
   e = (stbtt__edge *) STBTT_malloc(sizeof(*e) * (unsigned long)(n+1), (size_t)userdata); // add an extra one as a sentinel
   if (e == 0) return;
   n = 0;
   ...
   STBTT_sort(e,(size_t) n, sizeof(e[0]), stbtt__edge_compare);
...
const char *stbtt_GetFontNameString(const stbtt_fontinfo *font, int *length, int platformID, int encodingID, int languageID, int nameID)
{
   ...
   stringOffset = (stbtt_int32)(nm + ttUSHORT(fc+nm+4));
   for (i=0; i < count; ++i) {
      stbtt_uint32 loc = nm + 6 + (stbtt_uint32)(12 * i);
...
static int stbtt__matchpair(stbtt_uint8 *fc, stbtt_uint32 nm, stbtt_uint8 *name, stbtt_int32 nlen, stbtt_int32 target_id, stbtt_int32 next_id)
{
   ...
   stbtt_int32 stringOffset = (stbtt_int32)(nm + ttUSHORT(fc+nm+4));

   for (i=0; i < count; ++i) {
      stbtt_uint32 loc = nm + 6 + 12 * (stbtt_uint32)i;
         ...
         if (platform == 0 || (platform == 3 && encoding == 1) || (platform == 3 && encoding == 10)) {
            ...
            if (matchlen >= 0) {
               ...
               if (i+1 < count && ttUSHORT(fc+loc+12+6) == next_id && ttUSHORT(fc+loc+12) == platform && ttUSHORT(fc+loc+12+2) == encoding && ttUSHORT(fc+loc+12+4) == language) {
                  stbtt_int32 s= ttUSHORT(fc+loc+12+8), o= ttUSHORT(fc+loc+12+10);
                  if (s== 0) {
                     if (matchlen == nlen)
                        return 1;
                  } else if (matchlen < nlen && name[matchlen] == ' ') {
                     ++matchlen;
                     if (stbtt_CompareUTF8toUTF16_bigendian((char*) (name+matchlen), nlen-matchlen, (char*)(fc+stringOffset+o),s))
                        return 1;
                  }
               } else {
