...
namespace fea
{
    ...
    class FEA_API Shader
    {
        public:
            ...
            void setVertexAttribute(const std::string& name, const uint32_t floatAmount, const float* data, int32_t count);
            void compile();
        private:
            GLuint mProgramId;
            std::unordered_map<std::string, GLint> mUniformLocations;
            std::unordered_map<std::string, GLint> mVertexAttributeLocations;
            std::unordered_map<std::string, GLuint> mVertexAttributeBuffers;
     ...
     ***
     *  @fn void Shader::setVertexAttribute(const std::string& name, const uint32_t floatAmount, const float* data, int32_t count) const
     *  @brief Set a vertex attribute in the shader.
     *  @param name Name of the attribute to set.
     *  @param floatAmount Amount of floats in the attribute.
     *  @param data Data array.
     *  @param count Amount of elements in data array.
     ***
